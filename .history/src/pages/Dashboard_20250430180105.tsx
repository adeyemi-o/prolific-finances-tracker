...
const Dashboard = () => {
  ...
  const processData = (data: Transaction[]) => {
    ...
    data.forEach(tx => {
      ...
      if (txDate >= firstDayCurrentMonth && txDate < firstDayNextMonth) {
        if (tx.type === 'Income') {
      ...
      else if (txDate >= firstDayPreviousMonth && txDate < firstDayCurrentMonth) {
        if (tx.type === 'Income') {
          previousMonthRevenue += tx.amount;
        } else if (tx.type === 'Expense') {
    ...
    // Calculate overall totals (can use all data or just current month, let's use current for consistency)
    const totalRevenue = currentMonthRevenue;
    const totalExpenses = currentMonthExpenses;
    const netProfit = currentMonthNetProfit;

    // Calculate percentage changes
    const calculateChange = (current: number, previous: number): number | null => {
      if (previous === 0) {
        return current === 0 ? 0 : null; // Indicate infinite increase if current > 0, or 0% if both are 0
      }
    ...
    // For net profit change, handle cases where previous profit was negative or zero carefully
    let netProfitChange: number | null = null;
    if (previousMonthNetProfit !== 0) {
       netProfitChange = ((currentMonthNetProfit - previousMonthNetProfit) / Math.abs(previousMonthNetProfit)) * 100;
    } else if (currentMonthNetProfit > 0) {
       netProfitChange = null; // Or treat as infinite increase
    } else {
       netProfitChange = 0; // Both zero or previous zero and current negative/zero
    }
    ...
    // Keep existing expense breakdown and monthly chart logic (adjust if needed based on new summary focus)
    const expensesByCategory: Record<string, number> = {};
    data.filter(tx => tx.type === 'Expense' && tx.date >= firstDayCurrentMonth && tx.date < firstDayNextMonth) // Filter for current month expenses
      .forEach(tx => {
