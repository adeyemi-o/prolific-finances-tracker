  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        ...
        // Wrap each major operation in try/catch to prevent cascading failures
        try {
          await ensureAdminUser("prolifichcs@gmail.com");
        } catch (error) {
          console.warn("Non-critical error ensuring admin for prolifichcs:", error);
          // Continue with other operations instead of stopping
        }
        ...
        try {
          ...
          console.warn("Non-critical error synchronizing user roles:", error);
          // Continue loading users even if roles synchronization fails
        }
        
        // Even if the above operations fail, still try to load users
        try {
